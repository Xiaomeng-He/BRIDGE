...
def train_evaluate_LSTM(dataset_name,
    ...
    hp = read_config(BASE_DIR / "LSTM_hparams.yml")
    dataset_hparams = hp[dataset_name]

    hidden_size = dataset_hparams['hidden_size']
    dropout = dataset_hparams['dropout']
    ...
    # training
    set_seed(seed)
    parameter_path = dir / 'LSTM_parameters.pt'
    model = ED_LSTM(act_embed, cat_dims, emb_dims, num_act,
                enc_input_size, hidden_size, num_layers, dropout,
                dec_input_size).to(device)

    model.apply(init_weights_uni)
