...
class BatchInference():
    ...
    def edit_suffix_acts(self):
        ...
        # (idx 3) is predicted, 0 otherwise. 
        end_pred_tens = (self.suffix_acts_decoded== 3).to(torch.int64) # (batch_size, window_size)
        ...
        pred_length = torch.argmax((self.suffix_acts_decoded== 3).to(torch.int64), dim=-1) 
    ...
    def get_actual_length(self):
        ...
        decoding step. 
        """
        actual_length = torch.argmax((self.act_labels == 3).to(torch.int64), dim=-1) # (batch_size,)
